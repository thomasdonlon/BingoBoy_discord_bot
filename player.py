...
async def increment_task(state, task_name, n=1, log_task=True, from_strength_potion=False):
    ...
    if task_name[0] != 'b':
        ...
        if await inventory_contains(state, 'e7') and await random_with_bonus(state) < 0.10:
...
async def log_task(state, task_name, rune_of_rep=False):
    ...
    if task_name[0] == 'c' and strength_level >= 3:
        if await random_with_bonus(state) < 0.25:
            extra_task = True
...
async def allocate_skill_points(state, skill_name, number): #TODO: on level up, when they get a new skill, say that they learned a new skill and display that skill's text
    ...
    if old_skill_level == 35:
        await ctx_print(state, f"{skill_name} is already at max level!")
        return

    elif old_skill_level + number > 35:
        number = 35 - old_skill_level
        await ctx_print(state, f"Skill levels can only be increased up to 35. Only {number} skill points have been spent.")
...
async def progress_quest(state, skip_task_check=False):
    ...
    strength_level = await get_player_x(state, 'strength_level')
    wisdom_level = await get_player_x(state, 'wisdom_level')
    if strength_level >= 5 and await random_with_bonus(state) < 0.3:
        await award_xp(state, 10)
    ...
    if complete_result is None and wisdom_level >= 15 and await random_with_bonus(state) < 0.20:
    ...
    if not complete_result and await inventory_contains(state, 'h5') and await random_with_bonus(state) < 0.3 and not skip_task_check:
        complete_result = await progress_quest(state, skip_task_check=True) # skip the task check since this is a bonus step
...
async def complete_sidequest(state, task_type, skip_task_check=False):
    ...
    if agility_level >= 1 and await random_with_bonus(state) < 0.10:
    ...
    if agility_level >= 20 and await random_with_bonus(state) < 0.20:
    ...
    if agility_level >= 28 and await random_with_bonus(state) < 0.20:
